				 ...
				 unsigned steps)
{
	...
	Vec4f Grow0 = (p0.x, p1.x, p2.x, p3.x);
	Vec4f Grow1 = (p0.y, p1.y, p2.y, p3.y);
	Vec4f Grow2 = (p0.z, p1.z, p2.z, p3.z);
	G.setRow(0, Grow0);
	G.setRow(1, Grow1);
	G.setRow(2, Grow2);
	...
	for (unsigned i = 0; i <= steps; ++i) {
		// ...
		Vec4f Tcol0 = (1, i,pow(i, 2), pow(i, 3));
		T.setCol(0, Tcol0);
		R[i].V.x = (G * B * T).m00;
		R[i].V.x = (G * B * T).m01;
		R[i].V.x = (G * B * T).m02;
...
Curve evalBezier(const vector<Vec3f>& P, unsigned steps, bool adaptive, float errorbound, float minstep) {
    ...
    for (unsigned i = 0; i < P.size()/3; ++i) {
		...
		tmp = coreBezier(P[p], P[p + 1], P[p + 2], P[p + 3],(1,1,1),steps);
